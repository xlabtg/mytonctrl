...
Self-check report: bug-bounty-reports/F-01-mytoncore-db-world-readable/self-check-report.md.

## Repository State

- Analysis date UTC: 2026-06-26T10:26:03Z
- Input: bug-bounty-reports/F-01-mytoncore-db-world-readable/report.md
...
The report claims that `mytoncore.db` is created with mode `0644` under umask `022`, exposing the alert bot token and node configuration metadata to other local users.

## Vulnerability Existence

- Exact files/functions: `mypylib/mypylib.py`, `MyPyClass._write_file_atomic`, `MyPyClass.write_db`, `MyPyClass.lock_file`.
- Verified code path: upstream `master` creates the database temporary file with `open(tmp_path, 'wt')` and replaces the target path; the lock file is created with `os.open(..., 0o644)`.
- Attacker-controlled input path: no remote attacker-controlled input path; exploitation is a local filesystem read after MyTonCtrl creates the file.
...
- Security impact: verified local exposure of the alert bot token and node metadata.
...
- Claimed impact/severity: Medium, local sensitive data exposure.
- Validated impact: Medium for local-host threat model; no remote impact validated.
- Overclaiming or downgrade notes: do not present this as remote compromise, RCE or validator-to-validator impact.
